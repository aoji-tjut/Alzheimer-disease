...
from keras.layers import Input, LeakyReLU, Add, UpSampling3D, Activation, SpatialDropout3D, Conv3D, Reshape, \
    GlobalAvgPool3D, GlobalMaxPool3D, Activation, ReLU, Lambda, Concatenate, Multiply, Softmax, Dot
...
def GlobalAttention(x):
    ...
    reshape = Reshape([c, h * w * d])(x)

    conv = Conv3D(1, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding="same", activation="softmax")(x)
    conv = Reshape([h * w * d, 1, 1, 1])(conv)
    soft = Softmax()(conv)

    mul = Dot(axes=(2, 1))([reshape, soft])

    conv = Conv3D(c // r, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding="same")(mul)
    relu = ReLU()(conv)
    in_normal = InstanceNormalization(axis=1)(relu)
    conv = Conv3D(c, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding="same")(in_normal)
...
def isensee2017_model(input_shape=(4, 128, 128, 128), n_base_filters=16, depth=5, dropout_rate=0.3,
    ...
    for level_number in range(depth):
        ...
        #add = Add()([dropout_conv1, dropout_conv3, dropout_conv5])

        conv1 = create_convolution_block(res, n_level_filters, kernel=(1, 1, 1), strides=(1, 1, 1))
        conv1 = create_convolution_block(conv1, n_level_filters, kernel=(1, 1, 1), strides=(1, 1, 1))
        conv1 = create_convolution_block(conv1, n_level_filters, kernel=(1, 1, 1), strides=(1, 1, 1))
        conv3 = create_convolution_block(res, n_level_filters, kernel=(1, 1, 1), strides=(1, 1, 1))
        conv3 = create_convolution_block(conv3, n_level_filters, kernel=(3, 3, 3), strides=(1, 1, 1))
        conv3 = create_convolution_block(conv3, n_level_filters, kernel=(3, 3, 3), strides=(1, 1, 1))
        conv5 = create_convolution_block(res, n_level_filters, kernel=(1, 1, 1), strides=(1, 1, 1))
        conv5 = create_convolution_block(conv5, n_level_filters, kernel=(5, 5, 5), strides=(1, 1, 1))
        conv5 = create_convolution_block(conv5, n_level_filters, kernel=(5, 5, 5), strides=(1, 1, 1))
    ...
    for level_number in range(depth - 2, -1, -1):
        ...
        #out_global = GlobalAttention(localization_output)
        #add = Add()([localization_output, out_global])
        #current_layer = add
